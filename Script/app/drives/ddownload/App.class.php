...
class App extends BaseController {
    ...
    protected string $baseUrl = 'https://api-v2.ddownload.com/api';
    private array $authData = [];

    public function __construct(CloudDrives $baseDrive){
        parent::__construct($baseDrive);
        $this->authErrorIdentity = ['Wrong auth','Invalid key'];
    }
    ...
    public function call($method = 'get', $path = '', $data = [], $isFilter = true){
        ...
        if(!empty($resp)){
            ...
            $results = $isFilter && isset($resp['result']) ? $resp['result'] : $resp;
